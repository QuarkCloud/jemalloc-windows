...
seq_store_##short_type(seq_##short_type##_t *dst, type *src) {		\
	...
	size_t old_seq = atomic_load_u32(&dst->seq, ATOMIC_RELAXED);	\
	atomic_store_u32(&dst->seq, old_seq + 1, ATOMIC_RELAXED);	\
	atomic_fence(ATOMIC_RELEASE);					\
	for (size_t i = 0; i < sizeof(buf) / sizeof(size_t); i++) {	\
		atomic_store_u32(&dst->data[i], buf[i], ATOMIC_RELAXED);	\
	}								\
	atomic_store_u32(&dst->seq, old_seq + 2, ATOMIC_RELEASE);	\
}									\
									\
...
seq_try_load_##short_type(type *dst, seq_##short_type##_t *src) {	\
	...
	size_t seq1 = atomic_load_u32(&src->seq, ATOMIC_ACQUIRE);	\
	if (seq1 % 2 != 0) {						\
		return false;						\
	}								\
	for (size_t i = 0; i < sizeof(buf) / sizeof(size_t); i++) {	\
		buf[i] = atomic_load_u32(&src->data[i], ATOMIC_RELAXED);	\
	}								\
	atomic_fence(ATOMIC_ACQUIRE);					\
	size_t seq2 = atomic_load_u32(&src->seq, ATOMIC_RELAXED);	\
	if (seq1 != seq2) {						\
		return false;						\
	}								\
